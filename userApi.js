...
const forgotPass = (req) => {
  ...
  return new Promise((resolve) => {
    ...
    sendEmail(
      ...
      payload,
      (emailResp, mailError) => {
        const queryString = `UPDATE userDetails SET token='${token}', tokentype='reset'  WHERE email = '${loginId}'`;
        if (!mailError) {
...
const resetPass = (req) => {
  ...
  return new Promise((resolve) => {
    dbConnection.query(queryString, function (err, result) {
      ...
      if (!result.length) {
        resolve({ loginResponseStatus: "INVALID_URL" });
      } else {
        const updatePassQuery = `UPDATE userDetails SET token=${null}, tokentype=${null}, password='${password}'  WHERE email = '${
          result[0].email
        }'`;
        dbConnection.query(updatePassQuery, function (err, updateResp) {
          if (err) {
            console.log("DB_ERROR" + JSON.stringify(err));
            resolve({ loginResponseStatus: err.code });
          } else if (!updateResp.affectedRows) {
            resolve({ loginResponseStatus: "INVALID_URL" });
          } else {
            resolve({
              loginResponseStatus: "SUCCESS",
